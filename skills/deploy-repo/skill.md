...
| User says | Mode |
|-----------|------|
| `@deploy-repo clone - <url> <target>` | Clone the framework repo into a standalone copy |
| `@deploy-repo archive - <target-dir>` | Write a tar archive of the framework for transfer or backup |

## When to use this instead of the others

| Skill | Result |
|-------|--------|
| `@deploy-basic` | Target holds `.cursorrules` + `.training.cto/`; skills read from the source. **Preferred for multi-repo sharing** |
| `@deploy-files` | Target holds its own `.ai.cto/` copy alongside its own app code |
| `@deploy-repo` | A standalone copy of the framework *as its own repo*, or a backup artifact |

Use `deploy-repo` when the framework itself is the deliverable — onboarding another machine,
forking for modification, or archiving a known-good version. It does not scaffold
`.training.cto/` into an application repo; that is what the other two do.
...
3. Run the script.
4. Report what landed and which follow-up applies — a clone is a *source*, so the next step is usually `@deploy-basic - /path/to/real/project` from inside it.
...
| `archive` | A tar archive in `<target-dir>`; no git history, no `.training.cto/` |

Neither mode writes `.cursorrules` or `.training.cto/` into an application repo.
...
`.training.cto/` — learner memory is not replaceable from a remote.
